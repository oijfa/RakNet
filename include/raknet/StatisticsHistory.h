...
/// \brief Input numerical values over time. Get sum, average, highest, lowest, standard deviation on recent or all-time values
    ...
    public:
        ...
        size_t GetObjectIndex(uint64_t objectId) const;
        bool AddValueByObjectID(uint64_t objectId, RakString key, SHValueType val, Time curTime, bool combineEqualTimes);
        void AddValueByIndex(size_t index, RakString key, SHValueType val, Time curTime, bool combineEqualTimes);
        ...
        struct TimeAndValueQueue
        {
            ...
            void ResizeSampleSet(int approximateSamples, DataStructures::Queue<StatisticsHistory::TimeAndValue> &blendedSamples,
