...
static const size_t MAX_UNSIGNED_LONG = 4294967295U;
...
namespace DataStructures
{
    ...
    public:
        ...
        list_type &operator[](size_t position) const;

        /// \brief Access an element by its index in the array.
        /// \param[in]  position The index into the array.
        /// \return The element at position \a position.
        list_type &Get(size_t position) const;
        ...
        void Insert(const list_type &input, size_t position);
        ...
        void Replace(const list_type &input, const list_type filler, size_t position);
        ...
        /// \param[in] position The index of the element to delete
        void RemoveAtIndexFast(size_t position);
    ...
    List<list_type>::List(const List &original_copy)
    {
        ...
        else
        {
            ...
            for (size_t counter = 0; counter < original_copy.list_size; ++counter)
    ...
    template<class list_type>
    inline list_type &List<list_type>::operator[](size_t position) const
    {
#ifdef _DEBUG
    ...
    template<class list_type>
    inline list_type &List<list_type>::Get(size_t position) const
    {
        return listArray[position];
    }
    ...
    template<class list_type>
    void List<list_type>::Insert(const list_type &input, size_t position)
    {
#ifdef _DEBUG
        if (position > list_size)
            RakAssert(position <= list_size);
        ...
        for (size_t counter = list_size; counter != position; counter--)
    ...
    template<class list_type>
    inline void List<list_type>::Replace(const list_type &input, const list_type filler, size_t position)
    {
        if ((list_size > 0) && (position < list_size))
            listArray[position] = input; // Direct replacement
        else
        {
    ...
    template<class list_type>
    void List<list_type>::RemoveAtIndex(size_t position)
    {
#ifdef _DEBUG
        if (position >= list_size)
        {
        ...
        {
            // Compress the array
            for (size_t counter = position; counter < list_size - 1; ++counter)
                listArray[counter] = listArray[counter + 1];
            // Don't call constructors, assignment operators, etc.
    ...
    template<class list_type>
    void List<list_type>::RemoveAtIndexFast(size_t position)
    {
#ifdef _DEBUG
        if (position >= list_size)
        {
    ...
    template<class list_type>
    inline void List<list_type>::RemoveFromEnd(size_t num)
    {
    ...
    {
        size_t amountToAllocate = allocation_size;
        if (allocation_size == 0)
            amountToAllocate = 16;
