...
namespace DataStructures
{
    ...
    RangeList<range_type>::Serialize(RakNet::BitStream *in, RakNet::BitSize_t maxBits, bool clearSerialized)
    {
        ...
        if (clearSerialized && countWritten)
        {
            ...
            for (size_t i = 0; i < rangeSize - countWritten; i++)
    ...
    void RangeList<range_type>::Insert(range_type index)
    {
        ...
        bool objectExists;
        size_t insertionIndex = ranges.GetIndexFromKey(index, &objectExists);
    ...
    template<class range_type>
    size_t RangeList<range_type>::Size(void) const
    {
        return ranges.Size();
    }

    template<class range_type>
    size_t RangeList<range_type>::RangeSum(void) const
    {
        size_t sum = 0;
        for (size_t i = 0; i < ranges.Size(); i++)
