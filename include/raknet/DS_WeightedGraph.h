...
namespace DataStructures
{
    ...
    public:
        ...
        size_t GetConnectionCount(size_t nodeIndex) const;
        void GetConnectionAtIndex(size_t nodeIndex, size_t connectionIndex, node_type &outNode, weight_type &outWeight) const;
        node_type GetNodeAtIndex(size_t nodeIndex) const;
...
#pragma warning( disable : 4127 ) // warning C4127: conditional expression is constant
#endif
                if (!allow_unlinkedNodes && adjacencyLists[i]->Size() == 0)
    ...
    bool WeightedGraph<node_type, weight_type, allow_unlinkedNodes>::HasConnection(const node_type &node1, const node_type &node2)
    {
        if (node1 == node2)
            return false;
        return adjacencyLists.Has(node1) && adjacencyLists.Get(node1)->Has(node2);
    }
    ...
    void WeightedGraph<node_type, weight_type, allow_unlinkedNodes>::AddConnection(const node_type &node1, const node_type &node2, weight_type weight)
    {
        ...
        if (!adjacencyLists.Has(node1))
            AddNode(node1);
        adjacencyLists.Get(node1)->Set(node2, weight);
        if (!adjacencyLists.Has(node2))
            AddNode(node2);
...
#pragma warning( disable : 4127 ) // warning C4127: conditional expression is constant
#endif
        if (!allow_unlinkedNodes) // If we do not allow _unlinked nodes, then if there are no connections, remove the node
        {
    ...
    template<class node_type, class weight_type, bool allow_unlinkedNodes>
    bool
    WeightedGraph<node_type, weight_type, allow_unlinkedNodes>::GetShortestPath(DataStructures::List<node_type> &path,
                                                                                ...
                                                                                weight_type INFINITE_WEIGHT)
    {
        ...
        if (!isValidPath || rootNode != startNode)
        {
            ClearDijkstra();
        ...
        // return the results
        bool objectExists;
        size_t col = costMatrixIndices.GetIndexFromKey(endNode, &objectExists);
        ...
        size_t row = costMatrixIndices.Size() - 2;
        if (row == 0)
        {
            path.Insert(startNode);
            path.Insert(endNode);
        ...
        weight_type currentWeight = costMatrix[row * adjacencyLists.Size() + col];
        if (currentWeight == INFINITE_WEIGHT)
        {
            // No path
            return true;
        }
        node_type vertex = endNode;
...
#pragma warning( disable : 4127 ) // warning C4127: conditional expression is constant
    ...
    template<class node_type, class weight_type, bool allow_unlinkedNodes>
    node_type WeightedGraph<node_type, weight_type, allow_unlinkedNodes>::GetNodeAtIndex(size_t nodeIndex) const
    {
        return adjacencyLists.GetKeyAtIndex(nodeIndex);
    }

    template<class node_type, class weight_type, bool allow_unlinkedNodes>
    size_t WeightedGraph<node_type, weight_type, allow_unlinkedNodes>::GetNodeCount(void) const
    {
        return adjacencyLists.Size();
    }

    template<class node_type, class weight_type, bool allow_unlinkedNodes>
    size_t WeightedGraph<node_type, weight_type, allow_unlinkedNodes>::GetConnectionCount(size_t nodeIndex) const
    {
        return adjacencyLists[nodeIndex]->Size();
    }

    template<class node_type, class weight_type, bool allow_unlinkedNodes>
    void WeightedGraph<node_type, weight_type, allow_unlinkedNodes>::GetConnectionAtIndex(size_t nodeIndex,
    ...
    bool WeightedGraph<node_type, weight_type, allow_unlinkedNodes>::GetSpanningTree(
            DataStructures::Tree<node_type> &outTree, DataStructures::List<node_type> *inputNodes, node_type startNode, weight_type INFINITE_WEIGHT)
    {
        ...
        {
            bool res = GetShortestPath(path, startNode, (*inputNodes)[i], INFINITE_WEIGHT);
            if (res && path.Size() > 0)
            {
                for (size_t j = 0; j < path.Size() - 1; j++)
                    outGraph.AddConnection(path[j], path[j + 1], INFINITE_WEIGHT); // Don't bother looking up the weight
            }
        }
        ...
        DataStructures::Tree<node_type> *current = &outTree;
        if (!outGraph.adjacencyLists.Has(startNode))
            return false;

        DataStructures::Map<node_type, weight_type> *adjacencyList = outGraph.adjacencyLists.Get(startNode);
        ...
        {
            NodeAndParent nap = nodesToProcess.Pop();
            current = nap.node;
            adjacencyList = outGraph.adjacencyLists.Get(nap.node->data);

            for (size_t i = 0; i < adjacencyList->Size(); i++)
            {
                node_type key = adjacencyList->GetKeyAtIndex(i);
    ...
    void WeightedGraph<node_type, weight_type, allow_unlinkedNodes>::GenerateDisjktraMatrix(node_type startNode, weight_type INFINITE_WEIGHT)
    {
        ...
        for (size_t col = 0; col < adjacencyLists.Size(); col++)
        {
        ...
        for (size_t col = 0; col < adjacencyLists.Size() * adjacencyLists.Size(); col++)
            costMatrix[col] = INFINITE_WEIGHT;

        node_type currentNode = startNode;
        size_t row = 0;
        weight_type currentNodeWeight = 0;
        rootNode = startNode;
        ...
        if (adjacencyLists.Size())
        {
            ...
            for (size_t row2 = 0; row2 < adjacencyLists.Size(); row2++)
        ...
        while (row < adjacencyLists.Size() - 1)
        {
            ...
            for (size_t col = 0; col < adjacencyList->Size(); col++)
            {
                weight_type edgeWeight = (*adjacencyList)[col];
                node_type adjacentKey = adjacencyList->GetKeyAtIndex(col);
                adjacentIndex = adjacencyLists.GetIndexAtKey(adjacentKey);
                weight_type adjacentNodeWeight = costMatrix[row * adjacencyLists.Size() + adjacentIndex];

                if (currentNodeWeight + edgeWeight < adjacentNodeWeight)
                {
                    // Update the weight for the adjacent node
                    for (size_t row2 = row; row2 < adjacencyLists.Size(); row2++)
            ...
            minHeap.Clear(true);
            for (size_t openSetIndex = 0; openSetIndex < openSet.Size(); openSetIndex++)
