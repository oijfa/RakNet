...
public:
    ...
    void RemoveEntry(void *entry, float minX, float minY, float maxX, float maxY);

    // Adds and removes in one pass, more efficient than calling both functions consecutively
    void MoveEntry(void *entry, float sourceMinX, float sourceMinY, float sourceMaxX, float sourceMaxY,
...
protected:
    unsigned WorldToCellX(float input) const;
    unsigned WorldToCellY(float input) const;
    unsigned WorldToCellXOffsetAndClamped(float input) const;
    unsigned WorldToCellYOffsetAndClamped(float input) const;
    ...
    unsigned gridCellWidthCount, gridCellHeightCount;
