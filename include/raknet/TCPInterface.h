...
/// \brief As the name says, a simple multithreaded TCP server.  Used by TelnetTransport
    ...
    public:
        ...
        SystemAddress
        Connect(const char *host, unsigned short remotePort, bool block = true, unsigned short socketFamily = AF_INET,
        ...
        /// Sends a byte stream
        virtual void Send(const char *data, size_t length, const SystemAddress &systemAddress, bool broadcast);

        // Sends a concatenated list of byte streams
        virtual bool SendList(const char **data, const size_t *lengths, size_t numParameters, const SystemAddress &systemAddress, bool broadcast);
        ...
        virtual void CloseConnection(SystemAddress systemAddress);
        ...
        virtual SystemAddress HasCompletedConnectionAttempt(void);

        /// Has a previous call to connect failed?
        /// \return UNASSIGNED_SYSTEM_ADDRESS = no. Anything else means yes.
        virtual SystemAddress HasFailedConnectionAttempt(void);

        /// Queued events of new incoming connections
        virtual SystemAddress HasNewIncomingConnection(void);

        /// Queued events of lost connections
        virtual SystemAddress HasLostConnection(void);
...
/// Stores information about a remote client.
    struct RemoteClient
    {
        ...
        int Recv(char *data, const size_t dataSize);
#endif
        ...
        void SendOrBuffer(const char **data, const size_t *lengths, size_t numParameters);
    };
