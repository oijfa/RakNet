...
/// \ingroup TEAM_BALANCER_GROUP
    ...
    protected:
        ...
        size_t GetMemberIndex(NetworkID memberId, RakNetGUID guid) const;
        size_t AddTeamMember(const TeamMember &tm); // Returns index of new member
        void RemoveTeamMember(size_t index);
        void EvenTeams(void);
        size_t GetMemberIndexToSwitchTeams(const DataStructures::List<TeamId> &sourceTeamNumbers, TeamId targetTeamNumber);
        void GetOverpopulatedTeams(DataStructures::List<TeamId> &overpopulatedTeams, int maxTeamSize);
        void SwitchMemberTeam(size_t teamMemberIndex, TeamId destinationTeam);
        void NotifyTeamAssigment(size_t teamMemberIndex);
        ...
        bool TeamWouldBeOverpopulatedOnAddition(TeamId teamId, size_t teamMemberSize); // Accounting for team balancing and team limits, would this team be overpopulated if a member was added to it?
        bool TeamWouldBeUnderpopulatedOnLeave(TeamId teamId, size_t teamMemberSize);
        ...
        void SwapTeamMembersByRequest(size_t memberIndex1, size_t memberIndex2);
