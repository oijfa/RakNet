...
PluginReceiveResult ConnectionGraph2::OnReceive(Packet *packet)
{
    ...
    {
        bool objectExists;
        size_t idx = remoteSystems.GetIndexFromKey(packet->guid, &objectExists);
        if (objectExists)
        {
    ...
    else if (packet->data[0] == ID_REMOTE_NEW_INCOMING_CONNECTION)
    {
        ...
        if (objectExists)
        {
            ...
            for (unsigned idx2 = 0; idx2 < numAddresses; idx2++)
            {
                SystemAddressAndGuid saag;
                bs.Read(saag.systemAddress);
                bs.Read(saag.guid);
                ...
                bool objectExists;
                size_t ii = remoteSystems[idx]->remoteConnections.GetIndexFromKey(saag, &objectExists);
                if (!objectExists)
