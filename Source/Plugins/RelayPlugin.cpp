...
void
RelayPlugin::SendGroupMessage(const RakNetGUID &relayPluginServerGuid, BitStream *bitStream, PacketPriority priority,
    ...
    if (strAndGuidSender)
    {
        ...
        for (unsigned int i = 0; i < chatRooms.Size(); i++)
        {
            if (chatRooms[i]->roomName == roomName)
                return JoinGroup(chatRooms[i], strAndGuidSender); // Join existing room
        }
...
void RelayPlugin::LeaveGroup(StrAndGuidAndRoom **strAndGuidSender)
{
    ...
    for (size_t i = 0; i < chatRooms.Size(); i++)
    {
        if (chatRooms[i]->roomName == (*strAndGuidSender)->currentRoom)
        {
            ...
            for (size_t j = 0; j < room->usersInRoom.Size(); j++)
            {
...
void RelayPlugin::SendMessageToRoom(StrAndGuidAndRoom **strAndGuidSender, BitStream *message)
{
    ...
    for (size_t i = 0; i < chatRooms.Size(); i++)
    {
        if (chatRooms[i]->roomName == (*strAndGuidSender)->currentRoom)
        {
            ...
            RP_Group *room = chatRooms[i];
            for (i = 0; i < room->usersInRoom.Size(); i++)
            {
...
void RelayPlugin::OnJoinGroupRequestFromClient(Packet *packet)
{
    ...
    if (groupJoined)
    {
        ...
        for (size_t i = 0; i < groupJoined->usersInRoom.Size(); i++)
