...
void MessageFilter::SetAllowMessageID(bool allow, int messageIDStart, int messageIDEnd, int filterSetID)
{
    ...
    for (int i = messageIDStart; i <= messageIDEnd; ++i)
...
void MessageFilter::SetAllowRPC4(bool allow, const char *uniqueID, int filterSetID)
{
    FilterSet *filterSet = GetFilterSetByID(filterSetID);
    bool objectExists;
    size_t idx = filterSet->allowedRPC4.GetIndexFromKey(uniqueID, &objectExists);
    if (allow)
    {
        if (!objectExists)
        {
...
void MessageFilter::SetFilterMaxTime(TimeMS allowedTime, bool banOnExceed, RakNet::TimeMS banTimeMS, int filterSetID)
{
    FilterSet *filterSet = GetFilterSetByID(filterSetID);
    filterSet->maxMemberTimeMS = allowedTime;
...
void MessageFilter::SetSystemFilterSet(AddressOrGUID addressOrGUID, int filterSetID)
{
    ...
    RakAssert(!addressOrGUID.IsUndefined());
//    bool objectExists;
...
size_t MessageFilter::GetSystemCount(int filterSetID) const
{
    if (filterSetID == -1)
...
size_t MessageFilter::GetFilterSetCount(void) const
{
...
void MessageFilter::DeleteFilterSet(int filterSetID)
{
    FilterSet *filterSet;
    bool objectExists;
    size_t index = filterList.GetIndexFromKey(filterSetID, &objectExists);
    if (objectExists)
    {
...
PluginReceiveResult MessageFilter::OnReceive(Packet *packet)
{
    ...
    switch (packet->data[0])
    {
        ...
        default:
            ...
            DataStructures::HashIndex index = systemList.GetIndexOf(packet);
            if (index.IsInvalid())
                break;
