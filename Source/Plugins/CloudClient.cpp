...
void
CloudClient::Post(CloudKey *cloudKey, const unsigned char *data, uint32_t dataLengthBytes, RakNetGUID systemIdentifier)
{
    RakAssert(cloudKey);
...
bool
CloudClient::Get(CloudQuery *keyQuery, DataStructures::List<RakNetGUID> &specificSystems, RakNetGUID systemIdentifier)
{
    RakNet::BitStream bsOut;
    bsOut.Write((MessageID) ID_CLOUD_GET_REQUEST);
    keyQuery->Serialize(true, &bsOut);
...
void
CloudClient::Unsubscribe(DataStructures::List<CloudKey> &keys, DataStructures::List<CloudQueryRow *> &specificSystems, RakNetGUID systemIdentifier)
{
    RakNet::BitStream bsOut;
    bsOut.Write((MessageID) ID_CLOUD_UNSUBSCRIBE_REQUEST);
    RakAssert(keys.Size() < (uint16_t) -1);
...
void CloudClient::OnGetReponse(Packet *packet, CloudClientCallback *_callback, CloudAllocator *_allocator)
{
    ...
    {
        for (unsigned i = 0; i < cloudQueryResult.rowsReturned.Size(); i++)
        {
