...
bool
UDPProxyClient::RequestForwarding(SystemAddress proxyCoordinator, SystemAddress sourceAddress, RakNetGUID targetGuid,
...
{
    for (size_t idx1 = 0; idx1 < pingServerGroups.Size();)
    {
...
PluginReceiveResult UDPProxyClient::OnReceive(Packet *packet)
{
    ...
    {
        PingServerGroup *psg;
        for (size_t idx1 = 0; idx1 < pingServerGroups.Size(); idx1++)
        {
            psg = pingServerGroups[idx1];
            for (size_t idx2 = 0; idx2 < psg->serversToPing.Size(); idx2++)
            {
            ...
            case ID_UDP_PROXY_FORWARDING_NOTIFICATION:
            {
                ...
                switch (packet->data[1])
                {
                    ...
                    case ID_UDP_PROXY_IN_PROGRESS:
                    {
                        ...
                        {
                            if (resultHandler)
                                resultHandler->OnForwardingInProgress(serverIP.C_String(), forwardingPort, packet->systemAddress,
                                                                      senderAddress, targetAddress, targetGuid, this);
                        }
                        else
                        {
                            ...
                            if (resultHandler)
                                resultHandler->OnForwardingNotification(serverIP.C_String(), forwardingPort, packet->systemAddress,
                                                                        senderAddress, targetAddress, targetGuid, this);
                        }
                    }
                        break;
...
void UDPProxyClient::OnPingServers(Packet *packet)
{
    ...
    char ipStr[64];
    for (size_t serverListIndex = 0; serverListIndex < serverListSize; serverListIndex++)
    {
...
{
    for (size_t serversToPingIndex = 0; serversToPingIndex < serversToPing.Size(); serversToPingIndex++)
    {
...
void UDPProxyClient::PingServerGroup::SendPingedServersToCoordinator(RakPeerInterface *rakPeerInterface)
{
    ...
    for (size_t serversToPingIndex = 0; serversToPingIndex < serversToPingSize; serversToPingIndex++)
    {
...
{
    for (size_t i = 0; i < pingServerGroups.Size(); i++)
