...
static size_t globalRegistrationIndex = 0;
...
RPC4GlobalRegistration::RPC4GlobalRegistration(const char *uniqueID, void (*functionPointer)(RakNet::BitStream *userData,
...
bool RPC4::RegisterBlockingFunction(const char *uniqueID, void ( *functionPointer )(RakNet::BitStream *userData,
                                                                                    RakNet::BitStream *returnData, Packet *packet))
{
...
void RPC4::RegisterLocalCallback(const char *uniqueID, MessageID messageId)
{
    ...
    RakNet::RakString str = uniqueID;
    size_t index = localCallbacks.GetIndexFromKey(messageId, &objectExists);
    if (objectExists)
    {
    ...
    {
        LocalCallback *lc = new LocalCallback;
...
bool RPC4::UnregisterLocalCallback(const char *uniqueID, MessageID messageId)
{
    ...
    {
        LocalCallback *lc = localCallbacks[index];
        size_t index2 = lc->functions.GetIndexFromKey(RakString(uniqueID), &objectExists);
        if (objectExists)
        {
...
void
RPC4::Call(const char *uniqueID, RakNet::BitStream *bitStream, PacketPriority priority, PacketReliability reliability,
                  ...
                  bool broadcast, bool invokeLocal)
{
    ...
    if (invokeLocal)
    {
        ...
        Packet packet;
        packet.guid = rakPeerInterface->GetMyGUID();
        packet.systemAddress = rakPeerInterface->GetInternalID(UNASSIGNED_SYSTEM_ADDRESS);
        ...
        InvokeSignal(functionIndex, bsptr, &packet);
    }
}
...
void RPC4::InvokeSignal(DataStructures::HashIndex functionIndex, RakNet::BitStream *serializedParameters, Packet *packet)
{
    ...
    for (size_t i = 0; i < localSlot->slotObjects.Size(); i++)
    {
...
{
    for (size_t i = 0; i < globalRegistrationIndex; i++)
    {
...
PluginReceiveResult RPC4::OnReceive(Packet *packet)
{
    if (packet->data[0] == ID_RPC_PLUGIN)
    {
        ...
        if (packet->data[1] == ID_RPC4_CALL)
        {
            ...
            if (!isBlocking)
            {
                ...
                void (*fp)(RakNet::BitStream *, Packet *) = registeredNonblockingFunctions.ItemAtIndex(skhi);
            ...
            else
            {
                ...
                void (*fp)(RakNet::BitStream *, RakNet::BitStream *, Packet *) = registeredBlockingFunctions.ItemAtIndex(skhi);
                RakNet::BitStream returnData;
                bsIn.AlignReadToByteBoundary();
    ...
    bool objectExists;
    size_t index = localCallbacks.GetIndexFromKey(packet->data[0], &objectExists);
    if (objectExists)
    {
        LocalCallback *lc = localCallbacks[index];
        for (size_t index2 = 0; index2 < lc->functions.Size(); index2++)
        {
            RakNet::BitStream bsIn(packet->data, packet->length, false);

            DataStructures::HashIndex skhi = registeredNonblockingFunctions.GetIndexOf(
