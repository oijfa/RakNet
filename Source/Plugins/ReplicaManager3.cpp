...
void
ReplicaManager3::DeallocReplicaNoBroadcastDestruction(RakNet::Connection_RM3 *connection, RakNet::Replica3 *replica3)
{
...
RakNet::Connection_RM3 *ReplicaManager3::PopConnection(size_t index, WorldId worldId)
{
    ...
    for (size_t index2 = 0; index2 < replicaList.Size(); index2++)
    {
        ...
        {
            for (size_t index3 = 0; index3 < connection->queryToSerializeReplicaList.Size(); index3++)
            {
    ...
    for (size_t index2 = 0; index2 < destructionList.Size(); index2++)
    {
...
RakNet::Connection_RM3 *ReplicaManager3::PopConnection(RakNetGUID guid, WorldId worldId)
{
    ...
    for (size_t index = 0; index < world->connectionList.Size(); index++)
    {
...
void ReplicaManager3::Reference(RakNet::Replica3 *replica3, WorldId worldId)
{
    ...
    size_t index = ReferenceInternal(replica3, worldId);

    if (index != (size_t) -1)
    {
        for (size_t pushIdx = 0; pushIdx < world->connectionList.Size(); pushIdx++)
        {
...
size_t ReplicaManager3::ReferenceInternal(RakNet::Replica3 *replica3, WorldId worldId)
{
    RakAssert(worldsArray[worldId] != 0 && "World not in use");
    RM3World *world = worldsArray[worldId];

    size_t index = world->userReplicaList.GetIndexOf(replica3);
    if (index == (size_t) -1)
    {
...
void ReplicaManager3::Dereference(RakNet::Replica3 *replica3, WorldId worldId)
{
    ...
    for (size_t index = 0; index < world->userReplicaList.Size(); index++)
    {
    ...
    for (size_t index2 = 0; index2 < world->connectionList.Size(); index2++)
...
{
    for (size_t index = 0; index < replicaListIn.Size(); index++)
...
size_t ReplicaManager3::GetReplicaCount(WorldId worldId) const
{
    RakAssert(worldsArray[worldId] != 0 && "World not in use");
    RM3World *world = worldsArray[worldId];
...
size_t ReplicaManager3::GetConnectionCount(WorldId worldId) const
{
    RakAssert(worldsArray[worldId] != 0 && "World not in use");
    RM3World *world = worldsArray[worldId];
...
{
    for (size_t i = 0; i < worldsList.Size(); i++)
    {
...
void ReplicaManager3::RM3World::Clear(ReplicaManager3 *replicaManager3)
{
    ...
    for (size_t i = 0; i < userReplicaList.Size(); i++)
    {
...
void Connection_RM3::AutoConstructByQuery(ReplicaManager3 *replicaManager3, WorldId worldId)
{
        ...
        constructionMode == QUERY_REPLICA_FOR_CONSTRUCTION_AND_DESTRUCTION)
    {
        while (index < queryToConstructReplicaList.Size())
        {
            ...
            RM3ConstructionState constructionState = lsr->replica->QueryConstruction(this, replicaManager3);
    ...
    else if (constructionMode == QUERY_CONNECTION_FOR_REPLICA_LIST)
    {
        ...
        // Create new
        for (size_t idx2 = 0; idx2 < constructedReplicasCulled.Size(); idx2++)
            OnConstructToThisConnection(constructedReplicasCulled[idx2], replicaManager3);

        for (size_t idx2 = 0; idx2 < destroyedReplicasCulled.Size(); idx2++)
        {
            bool objectExists;
            size_t idx1 = constructedReplicaList.GetIndexFromKey(destroyedReplicasCulled[idx2], &objectExists);
            if (objectExists)
            {
                constructedReplicaList.RemoveAtIndex(idx1);

                for (size_t j = 0; j < queryToSerializeReplicaList.Size(); j++)
                {
...
void ReplicaManager3::Update(void)
{
    ...
    for (size_t index3 = 0; index3 < worldsList.Size(); index3++)
    {
...
PluginReceiveResult
ReplicaManager3::OnConstruction(Packet *packet, unsigned char *packetData, size_t packetDataLength, RakNetGUID senderGuid,
                                unsigned char packetDataOffset, WorldId worldId)
{
    ...
    uint16_t constructionObjectListSize;
    bsIn.Read(constructionObjectListSize);
    for (uint16_t index = 0; index < constructionObjectListSize; index++)
    {
        bsIn.Read(streamEnd);
        bsIn.Read(networkId);
        ...
        if (actuallyCreateObject)
        {
            ...
            bsIn.AlignReadToByteBoundary();
            Replica3 *replica = connection->AllocReplica(&bsIn, this);
            if (replica == 0)
            {
                constructionTickStack.Push(0);
                bsIn.SetReadOffset(streamEnd);
    ...
    for (uint16_t index = 0; index < constructionObjectListSize; index++)
    {
        if (constructionTickStack[index] != 0)
        {
            if (actuallyCreateObjectList[index])
            {
                ...
                for (uint16_t index2 = 0; index2 < world->connectionList.Size(); index2++)
                {
    ...
    // Destructions
    uint16_t destructionObjectListSize;
    bool b = bsIn.Read(destructionObjectListSize);
    RakAssert(b);
    for (uint16_t index = 0; index < destructionObjectListSize; index++)
    {
        bsIn.Read(networkId);
        bsIn.Read(streamEnd);
        Replica3 *replica = world->networkIDManager->GET_OBJECT_FROM_ID<Replica3 *>(networkId);
        if (replica == 0)
        {
            // Unknown object
            bsIn.SetReadOffset(streamEnd);
...
PluginReceiveResult ReplicaManager3::OnSerialize(Packet *packet, unsigned char *packetData, size_t packetDataLength, RakNetGUID senderGuid,
                                                 RakNet::Time timestamp, unsigned char packetDataOffset, WorldId worldId)
{
    Connection_RM3 *connection = GetConnectionByGUID(senderGuid, worldId);
    if (connection == 0)
        return RR_CONTINUE_PROCESSING;
...
PluginReceiveResult ReplicaManager3::OnDownloadStarted(Packet *packet, unsigned char *packetData, size_t packetDataLength,
...
PluginReceiveResult ReplicaManager3::OnDownloadComplete(Packet *packet, unsigned char *packetData, size_t packetDataLength,
                                                        RakNetGUID senderGuid, unsigned char packetDataOffset, WorldId worldId)
{
    Connection_RM3 *connection = GetConnectionByGUID(senderGuid, worldId);
    if (connection == 0)
    ...
    if (connection->groupConstructionAndSerialize && connection->downloadGroup.Size() > 0)
    {
        // Push back buffered packets in front of this one
        for (size_t i = 0; i < connection->downloadGroup.Size(); i++)
...
Replica3 *ReplicaManager3::GetReplicaByNetworkID(NetworkID networkId, WorldId worldId)
{
    ...
    for (size_t i = 0; i < world->userReplicaList.Size(); i++)
    {
                                               ...
                                               const SystemAddress &exclusionAddress, WorldId worldId)
{
    ...
    for (size_t i = 0; i < replicaListSource.Size(); i++)
    {
    ...
    for (size_t i = 0; i < replicaList.Size(); i++)
    {
    ...
    for (size_t j = 0; j < world->connectionList.Size(); j++)
    {
...
{
    for (size_t i = 0; i < constructedReplicaList.Size(); i++)
        delete constructedReplicaList[i];
    for (size_t i = 0; i < queryToConstructReplicaList.Size(); i++)
...
void Connection_RM3::GetConstructedReplicas(DataStructures::List<Replica3 *> &objectsTheyDoHave)
{
    ...
    for (size_t idx = 0; idx < constructedReplicaList.Size(); idx++)
...
{
    for (size_t i = 0; i < downloadGroup.Size(); i++)
                              ...
                              RakPeerInterface *rakPeer, unsigned char worldId, RakNet::Time curTime)
{
    ...
    for (int channelIndex = 0; channelIndex < RM3_NUM_OUTPUT_BITSTREAM_CHANNELS; channelIndex++)
    {
        if (channelIndex == 0)
...
SendSerializeIfChangedResult
Connection_RM3::SendSerializeIfChanged(LastSerializationResult *lsr, SerializeParameters *sp,
                                       ...
                                       ReplicaManager3 *replicaManager, RakNet::Time curTime)
{
    ...
    if (replica->forceSendUntilNextUpdate)
    {
        ...
        return SendSerialize(replica, replica->lastSentSerialization.indicesToSend, replica->lastSentSerialization.bitStream,
    ...
    if (serializationResult == RM3SR_DO_NOT_SERIALIZE)
        return SSICR_DID_NOT_SEND_DATA; // Don't serialize this tick only
    ...
    if (serializationResult == RM3SR_SERIALIZED_ALWAYS_IDENTICALLY)
    {
        ...
        return SendSerialize(replica, replica->lastSentSerialization.indicesToSend, sp->outputBitstream, sp->messageTimestamp,
...
void Connection_RM3::OnDereference(Replica3 *replica3, ReplicaManager3 *replicaManager)
{
    ...
    bool objectExists;
    size_t idx = constructedReplicaList.GetIndexFromKey(replica3, &objectExists);
    if (objectExists)
    {
...
void Connection_RM3::OnDownloadFromThisSystem(Replica3 *replica3, ReplicaManager3 *replicaManager)
{
    ...
    {
        for (size_t j = 0; j < queryToConstructReplicaList.Size(); j++)
        {
...
void Connection_RM3::OnNeverConstruct(size_t queryToConstructIdx, ReplicaManager3 *replicaManager)
{
    ConstructionMode constructionMode = QueryConstructionMode();
    RakAssert(constructionMode == QUERY_REPLICA_FOR_CONSTRUCTION ||
              constructionMode == QUERY_REPLICA_FOR_CONSTRUCTION_AND_DESTRUCTION);
...
void Connection_RM3::OnConstructToThisConnection(size_t queryToConstructIdx, ReplicaManager3 *replicaManager)
{
    ConstructionMode constructionMode = QueryConstructionMode();
...
void Connection_RM3::OnReplicaAlreadyExists(size_t queryToConstructIdx, ReplicaManager3 *replicaManager)
{
    ConstructionMode constructionMode = QueryConstructionMode();
    RakAssert(constructionMode == QUERY_REPLICA_FOR_CONSTRUCTION ||
              constructionMode == QUERY_REPLICA_FOR_CONSTRUCTION_AND_DESTRUCTION);
...
void Connection_RM3::OnDownloadExisting(Replica3 *replica3, ReplicaManager3 *replicaManager)
{
    ...
    if (constructionMode == QUERY_REPLICA_FOR_CONSTRUCTION ||
        constructionMode == QUERY_REPLICA_FOR_CONSTRUCTION_AND_DESTRUCTION)
    {
        for (size_t idx = 0; idx < queryToConstructReplicaList.Size(); idx++)
        {
            if (queryToConstructReplicaList[idx]->replica == replica3)
            {
...
void Connection_RM3::OnSendDestructionFromQuery(size_t queryToDestructIdx, ReplicaManager3 *replicaManager)
{
    ConstructionMode constructionMode = QueryConstructionMode();
    RakAssert(constructionMode == QUERY_REPLICA_FOR_CONSTRUCTION ||
              constructionMode == QUERY_REPLICA_FOR_CONSTRUCTION_AND_DESTRUCTION);
    ...
    for (size_t j = 0; j < constructedReplicaList.Size(); j++)
    {
...
void Connection_RM3::OnDoNotQueryDestruction(size_t queryToDestructIdx, ReplicaManager3 *replicaManager)
{
    ValidateLists(replicaManager);
    queryToDestructReplicaList.RemoveAtIndex(queryToDestructIdx);
    ValidateLists(replicaManager);
                                      ...
                                      ReplicaManager3 *replicaManager3)
{
    ...
    // Construction
    for (size_t newListIndex = 0; newListIndex < newObjects.Size(); newListIndex++)
    {
        offsetStart = bsOut.GetWriteOffset();
    ...
    bsOut.Write(objectSize);
    for (size_t oldListIndex = 0; oldListIndex < deletedObjects.Size(); oldListIndex++)
    {
