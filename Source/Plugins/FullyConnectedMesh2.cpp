...
int
FCM2ParticipantComp(FullyConnectedMesh2::FCM2Participant *const &key, FullyConnectedMesh2::FCM2Participant *const &data)
{
...
void FullyConnectedMesh2::GetHostOrder(DataStructures::List<RakNetGUID> &hostList)
{
    ...
    for (size_t i = 0; i < fcm2ParticipantList.Size(); i++)
...
void FullyConnectedMesh2::OnClosedConnection(const SystemAddress &systemAddress, RakNetGUID rakNetGUID, PI2_LostConnectionReason lostConnectionReason)
{
    ...
    for (size_t idx = 0; idx < joinsInProgress.Size();)
    {
        if (joinsInProgress[idx]->requester == rakNetGUID)
        {
            ...
            for (size_t j = 0; j < joinsInProgress[idx]->vjipMembers.Size(); j++)
            {
    ...
    for (size_t idx = 0; idx < fcm2ParticipantList.Size(); idx++)
    {
...
void FullyConnectedMesh2::Clear(void)
{
    ...
    for (size_t i = 0; i < joinsInProgress.Size(); i++)
    {
        for (size_t j = 0; j < joinsInProgress[i]->vjipMembers.Size(); j++)
        {
...
void FullyConnectedMesh2::SendConnectionCountResponse(SystemAddress addr, size_t responseTotalConnectionCount)
{
    RakNet::BitStream bsOut;
...
void FullyConnectedMesh2::AssignOurFCMGuid(void)
{
    ...
    randomNumber ^= (unsigned int) (rakPeerInterface->GetGuidFromSystemAddress(UNASSIGNED_SYSTEM_ADDRESS).g &
                                    0xFFFFFFFF);
...
void FullyConnectedMesh2::GetParticipantCount(size_t *participantListSize) const
{
    *participantListSize = fcm2ParticipantList.Size();
}

size_t FullyConnectedMesh2::GetParticipantCount(void) const
{
...
void FullyConnectedMesh2::IncrementTotalConnectionCount(size_t i)
{
...
void FullyConnectedMesh2::ConnectToRemoteNewIncomingConnections(Packet *packet)
{
    ...
    for (unsigned i = 0; i < count; i++)
    {
        ...
        rakPeerInterface->Connect(str, remoteAddress.GetPort(), connectionPassword.C_String(), connectionPassword.GetLength());
    }
}

size_t FullyConnectedMesh2::GetTotalConnectionCount(void) const
{
...
void FullyConnectedMesh2::StartVerifiedJoin(RakNetGUID client)
{
    ...
    RakAssert(!HasParticipant(client));
...
void FullyConnectedMesh2::RespondOnVerifiedJoinCapable(Packet *packet, bool accept, BitStream *additionalData)
{
    ...
    if (participatingMembersNotOnClient.Size() > 0)
    {
        ...
        for (size_t i = 0; i < participatingMembersNotOnClient.Size(); i++)
        {
            ...
            bool written = false;
            for (size_t j = 0; j < fcm2ParticipantList.Size(); j++)
            {
    ...
    if (accept)
    {
        ...
        for (size_t i = 0; i < clientMembersNotParticipatingSucceeded.Size(); i++)
        ...
        for (size_t i = 0; i < participatingMembersOnClientSucceeded.Size(); i++)
                                                                ...
                                                                DataStructures::List<BitStream *> &userData)
{
    ...
    size_t curIndex = GetJoinsInProgressIndex(host);
    if (curIndex != (size_t) -1)
    {
        VerifiedJoinInProgress *vjip = joinsInProgress[curIndex];
        for (size_t j = 0; j < vjip->vjipMembers.Size(); j++)
        {
...
PluginReceiveResult FullyConnectedMesh2::OnVerifiedJoinStart(Packet *packet)
{
    ...
    size_t curIndex = GetJoinsInProgressIndex(packet->guid);
    if (curIndex != (size_t) -1)
    {
...
//         }
        ...
        for (size_t i = 0; i < vjip->vjipMembers.Size(); i++)
        ...
        if (ProcessVerifiedJoinInProgressIfCompleted(vjip))
            return RR_STOP_PROCESSING_AND_DEALLOCATE; // Completed
...
PluginReceiveResult FullyConnectedMesh2::OnVerifiedJoinCapable(Packet *packet)
{
    ...
    RakAssert(!HasParticipant(packet->guid));
...
{
    size_t curIndex = GetJoinsInProgressIndex(hostGuid);
    if (curIndex == (size_t) -1)
        return;
    ...
    for (size_t j = 0; j < joinsInProgress[curIndex]->vjipMembers.Size(); j++)
    {
...
size_t FullyConnectedMesh2::GetJoinsInProgressIndex(RakNetGUID requester) const
{
    for (size_t i = 0; i < joinsInProgress.Size(); i++)
    {
...
void
FullyConnectedMesh2::UpdateVerifiedJoinInProgressMember(const AddressOrGUID systemIdentifier, RakNetGUID guidToAssign,
                                                        FullyConnectedMesh2::JoinInProgressState newState)
{
    for (size_t i = 0; i < joinsInProgress.Size(); i++)
    {
        ...
        size_t j = GetVerifiedJoinInProgressMemberIndex(systemIdentifier, vjip);
        if (j != (size_t) -1)
        {
...
size_t FullyConnectedMesh2::GetVerifiedJoinInProgressMemberIndex(const AddressOrGUID systemIdentifier, VerifiedJoinInProgress *vjip)
{
    for (size_t j = 0; j < vjip->vjipMembers.Size(); j++)
    {
                                         ...
                                         DataStructures::List<RakNetGUID> &clientMembersNotParticipatingFailed)
{
    ...
    for (size_t i = 0; i < fcm2ParticipantList.Size(); i++)
    {
        size_t j = GetVerifiedJoinInProgressMemberIndex(fcm2ParticipantList[i]->rakNetGuid, vjip);
        if (j == (size_t) -1)
