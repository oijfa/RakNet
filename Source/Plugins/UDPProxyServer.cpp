...
{
    bool objectExists;
    size_t insertionIndex = loggingInCoordinators.GetIndexFromKey(coordinatorAddress, &objectExists);
    if (objectExists)
        return false;
...
PluginReceiveResult UDPProxyServer::OnReceive(Packet *packet)
{
    ...
    if (packet->data[0] == ID_UDP_PROXY_GENERAL && packet->length > 1)
    {
        ...
        switch (packet->data[1])
        {
            ...
            {
                size_t removalIndex = loggingInCoordinators.GetIndexFromKey(packet->systemAddress, &objectExists);
                if (objectExists)
                {
