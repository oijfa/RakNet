...
public:
    ...
    virtual bool OnFile(OnFileStruct *onFileStruct)
    {
        ...
        if (onFileStruct->fileName && onFileStruct->fileData && subdirLen < strlen(onFileStruct->fileName))
        {
            ...
            WriteFileWithDirectories(fullPathToDir, onFileStruct->fileData, onFileStruct->byteLengthOfThisFile);
        }
        else
            fullPathToDir[0] = 0;
...
void DirectoryDeltaTransfer::SetFileListTransferPlugin(FileListTransfer *flt)
{
    if (fileListTransfer)
    {
        ...
        for (size_t i = 0; i < fileListProgressList.Size(); i++)
...
unsigned short
DirectoryDeltaTransfer::DownloadFromSubdirectory(FileList &localFiles, const char *subdir, const char *outputSubdir,
                                                 ...
                                                 char _orderingChannel, FileListProgress *cb)
{
    ...
    if (subdir && subdir[0])
    {
        transferCallback->subdirLen = strlen(subdir);
                                                                ...
                                                                FileListProgress *cb)
{
    ...
    localFiles.AddFilesFromDirectory(prependAppDirToOutputSubdir ? applicationDirectory : 0, outputSubdir, true, false, true,
...
void
DirectoryDeltaTransfer::GenerateHashes(FileList &localFiles, const char *outputSubdir, bool prependAppDirToOutputSubdir)
{
    localFiles.AddFilesFromDirectory(prependAppDirToOutputSubdir ? applicationDirectory : 0, outputSubdir, true, false, true,
                                     FileListNodeContext(0, 0, 0, 0));
}
...
{
    if (packet->data[0] == ID_DDT_DOWNLOAD_REQUEST)
    {
...
size_t DirectoryDeltaTransfer::GetNumberOfFilesForUpload() const
{
    return availableUploads->fileList.Size();
}

void
DirectoryDeltaTransfer::SetDownloadRequestIncrementalReadInterface(IncrementalReadInterface *_incrementalReadInterface, size_t _chunkSize)
{
