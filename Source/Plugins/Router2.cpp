...
void Router2::ClearConnectionRequests(void)
{
    ...
    for (size_t i = 0; i < connectionRequests.Size(); i++)
...
bool Router2::ConnectInternal(RakNetGUID endpointGuid, bool returnConnectionLostOnFailure)
{
    ...
    // ALready in progress?
    connectionRequestsMutex.Lock();
    if (GetConnectionRequestIndex(endpointGuid) != (size_t) -1)
    {
        connectionRequestsMutex.Unlock();
        char buff[512];
        if (debugInterface)
    ...
    if (guids.Size() == 0)
    {
        char buff[512];
        if (debugInterface)
            debugInterface->ShowFailure(FormatString(buff, 512, "Router2 failed at %s:%i\n"));
    ...
    for (size_t i = 0; i < guids.Size(); i++)
    {
        ...
        if (guids[i] != endpointGuid)
        {
            ...
            {
                char buff[512];
                debugInterface->ShowDiagnostic(FormatString(buff, 512, "Router2::ConnectInternal: at %s:%i, pack_id = %d",
        ...
        else
        {
            if (debugInterface)
            {
                ...
                debugInterface->ShowDiagnostic(
                        FormatString(buff, 512, "Router2::ConnectInternal: at %s:%i [else ..].: %I64d==%I64d",
        ...
        switch (packet->data[1])
        {
            case ID_ROUTER_2_REPLY_TO_SENDER_PORT:
            {
                ...
                if (debugInterface)
                {
                    ...
                    debugInterface->ShowDiagnostic(FormatString(buff, 512, "Got ID_ROUTER_2_REPLY_TO_SENDER_PORT %i on "
                                                                        "address %s, replying with ID_ROUTER_2_MINI_PUNCH_REPLY at %s:%i\n",
                                                                sa.GetPort(), buff2));
...
//        printf("Got ID_ROUTER_2_FORWARDING_ESTABLISHED\n");
    ...
    {
        size_t forwardingIndex = 0;
        forwardedConnectionListMutex.Lock();
        for (; forwardingIndex < forwardedConnectionList.Size(); forwardingIndex++)
        {
...
void Router2::Update(void)
{
    ...
    connectionRequestsMutex.Lock();
    for (size_t connectionRequestIndex = 0; connectionRequestIndex < connectionRequests.Size();)
    {
            ...
            connectionRequest->pingTimeout < curTime)
        {
            ...
            for (size_t requestGuidIndex = 0; requestGuidIndex < connectionRequest->connectionRequestSystems.Size();)
            {
                if (connectionRequest->connectionRequestSystems[requestGuidIndex].pingToEndpoint < 0)
                {
                    anyRemoved = true;
                    connectionRequest->connectionRequestSystems.RemoveAtIndexFast(requestGuidIndex);
                }
                else
                    requestGuidIndex++;
            }
            ...
            if (anyRemoved)
            {
                if (connectionRequestIndex != (size_t) -1)
                {
    ...
    miniPunchesInProgressMutex.Lock();
    for (size_t i = 0; i < miniPunchesInProgress.Size();)
    {
                                 ...
                                 PI2_LostConnectionReason lostConnectionReason)
{
    ...
    forwardedConnectionListMutex.Lock();
    for (size_t forwardedConnectionIndex = 0; forwardedConnectionIndex < forwardedConnectionList.Size();)
    {
        if (forwardedConnectionList[forwardedConnectionIndex].endpointGuid == rakNetGUID)
        {
            ...
            {
                char buff[512];
                debugInterface->ShowDiagnostic(FormatString(buff, 512,
                 ...
                 forwardedConnectionList[forwardedConnectionIndex].weInitiatedForwarding)
        {
            ...
            connectionRequestsMutex.Lock();
            size_t pos = GetConnectionRequestIndex(forwardedConnectionList[forwardedConnectionIndex].endpointGuid);
            if ((size_t) -1 != pos)
                delete connectionRequests[pos];
            connectionRequestsMutex.Unlock();
...
//            forwardedConnectionList.RemoveAtIndexFast(forwardedConnectionIndex);
    ...
    for (size_t connectionRequestIndex = 0; connectionRequestIndex < connectionRequests.Size();)
    {
        ...
        cr->connectionRequestSystemsMutex.Lock();
        size_t connectionRequestGuidIndex = cr->GetGuidIndex(rakNetGUID);
        if (connectionRequestGuidIndex != (size_t) -1)
        {
            ...
            if (!UpdateForwarding(cr)) // If returns false, no connection request systems left
            {
                ...
                {
                    char buff[512];
                    debugInterface->ShowDiagnostic(FormatString(buff, 512, "Aborted connection to the %I64d, "
                                                                        "aborted forwarding at %s:%i\n", rakNetGUID.g,
                                                                __FILE__, __LINE__));
                }
            ...
            else // Else a system in the connection request list dropped. If cr->requestState==R2RS_REQUEST_STATE_QUERY_FORWARDING then we are still waiting for other systems to respond.
            {
                ...
                {
                    char buff[512];
                    debugInterface->ShowDiagnostic(FormatString(buff, 512, "Aborted connection attempt to %I64d, restarting "
                                                                        "forwarding to %I64d at %s:%i\n", rakNetGUID.g,
                                                                cr->endpointGuid.g));
                }
...
bool Router2::UpdateForwarding(ConnnectRequest *connectionRequest)
{
    ...
    if (connectionRequest->connectionRequestSystems.Size() == 0)
    {
        ...
        if (connectionRequest->returnConnectionLostOnFailure)
        {
            ...
            ReturnToUser(ID_CONNECTION_LOST, connectionRequest->endpointGuid, UNASSIGNED_SYSTEM_ADDRESS, true);
        ...
        for (size_t connectionIndex = 0; connectionIndex < forwardedConnectionList.Size(); connectionIndex++)
        {
            if (forwardedConnectionList[connectionIndex].endpointGuid == connectionRequest->endpointGuid &&
                forwardedConnectionList[connectionIndex].weInitiatedForwarding)
            {
                forwardedConnectionList.RemoveAtIndexFast(connectionIndex);
                break;
            }
        }
        forwardedConnectionListMutex.Unlock();
    ...
    if (connectionRequest->requestState == R2RS_REQUEST_STATE_QUERY_FORWARDING)
    {
        ...
        for (size_t i = 0; i < connectionRequest->connectionRequestSystems.Size(); i++)
        {
...
// connectionRequestsMutex should already be locked
void Router2::RemoveConnectionRequest(size_t connectionRequestIndex)
{
...
int
ConnectionRequestSystemComp(const Router2::ConnectionRequestSystem &key, const Router2::ConnectionRequestSystem &data)
{
...
void Router2::RequestForwarding(ConnnectRequest *connectionRequest)
{
    ...
    if (connectionRequest->GetGuidIndex(connectionRequest->lastRequestedForwardingSystem) != (size_t) -1)
    {
        char buff[512];
        if (debugInterface)
            debugInterface->ShowFailure(FormatString(buff, 512, "Router2 failed at %s:%i\n"));
    ...
    connectionRequest->connectionRequestSystemsMutex.Lock();
    for (size_t connectionRequestGuidIndex = 0;
         connectionRequestGuidIndex < connectionRequest->connectionRequestSystems.Size(); connectionRequestGuidIndex++)
    {
...
int Router2::ReturnFailureOnCannotForward(RakNetGUID sourceGuid, RakNetGUID endpointGuid)
{
    ...
    if (udpForwarder == 0 || udpForwarder->GetUsedForwardEntries() / 2 > maximumForwardingRequests)
    {
        char buff[512];
        if (debugInterface)
            debugInterface->ShowFailure(FormatString(buff, 512, "Router2 failed (%I64d -> %I64d) at %s:%i\n",
    ...
    forwardedConnectionListMutex.Lock();
    for (size_t i = 0; i < forwardedConnectionList.Size(); i++)
    {
        // sourceGuid is here so you do not respond to routing requests from systems you are already routing through.
        if ((forwardedConnectionList[i].endpointGuid == endpointGuid)
            || (forwardedConnectionList[i].endpointGuid == sourceGuid))
        {
            forwardedConnectionListMutex.Unlock();
...
void Router2::OnQueryForwarding(Packet *packet)
{
    ...
    if (pingToEndpoint == -1)
    {
        ...
        if (debugInterface)
            debugInterface->ShowFailure(
                    FormatString(buff, 512, "Router2 failed (%I64d) at %s:%i\n", packet->guid.g));
        return;
    }
    ...
    if (debugInterface)
    {
        char buff[512];
        debugInterface->ShowDiagnostic(
                FormatString(buff, 512, "Sending ID_ROUTER_2_REPLY_FORWARDING to the %I64d at %s:%i\n", packet->guid.g,
                             __FILE__, __LINE__));
    }
}
...
void Router2::OnQueryForwardingReply(Packet *packet)
{
    ...
    connectionRequestsMutex.Lock();
    size_t connectionRequestIndex = GetConnectionRequestIndex(endpointGuid);
    if (connectionRequestIndex == (size_t) -1)
    {
        connectionRequestsMutex.Unlock();
        char buff[512];
        if (debugInterface)
            debugInterface->ShowFailure(FormatString(buff, 512, "Router2 failed (%I64d) at %s:%i\n",
                                                     endpointGuid.g));
        return;
    }

    connectionRequests[connectionRequestIndex]->connectionRequestSystemsMutex.Lock();
    size_t connectionRequestGuidIndex = connectionRequests[connectionRequestIndex]->GetGuidIndex(packet->guid);
    if (connectionRequestGuidIndex == (size_t) -1)
    {
    ...
    if (debugInterface)
    {
        ...
        char buff2[512];
        packet->systemAddress.ToString(true, buff2);
        debugInterface->ShowDiagnostic(FormatString(buff, 512, "Got ID_ROUTER_2_REPLY_FORWARDING on address %s(%I64d "
                                                            "-> %I64d) canForward=%i at %s:%i\n", buff2, packet->guid.g,
                                                    endpointGuid.g, canForward));
    }
                                    ...
                                    unsigned short sourceToDstPort)
{
    ...
    if (debugInterface)
    {
        ...
        if (messageId == ID_ROUTER_2_FORWARDING_ESTABLISHED)
            debugInterface->ShowDiagnostic(
                    FormatString(buff, 512, "Sending ID_ROUTER_2_FORWARDING_ESTABLISHED at %s:%i\n"));
        else
            debugInterface->ShowDiagnostic(
                    FormatString(buff, 512, "Sending ID_ROUTER_2_REROUTED at %s:%i\n"));
    }
}
...
void Router2::OnRequestForwarding(Packet *packet)
{
    ...
    if (pingToEndpoint == -1)
    {
        ...
        if (debugInterface)
            debugInterface->ShowFailure(
                    FormatString(buff, 512, "Router2 failed (packet->guid =%I64d, endpointGuid = %I64d) at %s:%i\n",
                                 packet->guid.g, endpointGuid.g));
        return;
    }
    ...
    UDPForwarderResult result = udpForwarder->StartForwarding(packet->systemAddress, endpointSystemAddress,
                                                              30000, 0, socketFamily, &forwardingPort, &forwardingSocket);
    ...
    else if (result == UDPFORWARDER_NO_SOCKETS)
    {
        ...
        if (debugInterface)
            debugInterface->ShowFailure(FormatString(buff, 512,
                                                     ...
                                                     __FILE__, __LINE__, buff2, buff3, forwardingPort,
                                                     forwardingSocket));
...
void Router2::OnMiniPunchReplyBounce(Packet *packet)
{
    ...
    {
        char buff[512];
        debugInterface->ShowDiagnostic(FormatString(buff, 512, "Got ID_ROUTER_2_MINI_PUNCH_REPLY_BOUNCE from guid=%I64d"
    ...
    while (i < miniPunchesInProgress.Size())
    {
            ...
            miniPunchesInProgress[i].endpointGuid == packet->guid)
        {
            ...
            if (miniPunchesInProgress[i].gotReplyFromEndpoint && miniPunchesInProgress[i].gotReplyFromSource)
            {
...
void Router2::OnRerouted(Packet *packet)
{
    ...
    else
    {
        ...
        {
            char buff[512];
            debugInterface->ShowDiagnostic(FormatString(buff, 512, "Got ID_ROUTER_2_REROUTE, returning ID_ROUTER_2_REROUTED,"
...
bool Router2::OnForwardingSuccess(Packet *packet)
{
    ...
    if (forwardingIndex < forwardedConnectionList.Size())
    {
        ...
        {
            char buff[512];
            debugInterface->ShowDiagnostic(FormatString(buff, 512, "Got ID_ROUTER_2_FORWARDING_ESTABLISHED, returning ID_ROUTER_2_REROUTED,"
                                                                " Calling RakPeer::ChangeSystemAddress at %s:%i\n"));
        }
    ...
    else
    {
        ...
        ForwardedConnection fc;
        connectionRequestsMutex.Lock();
        unsigned connectionRequestIndex = GetConnectionRequestIndex(endpointGuid);
        ...
        if (debugInterface)
        {
            char buff[512];
            debugInterface->ShowDiagnostic(
                    FormatString(buff, 512, "Got and returning to user ID_ROUTER_2_FORWARDING_ESTABLISHED at %s:%i\n"));
        }
...
{
    int largestPing = -1;
    unsigned maxPeers = rakPeerInterface->GetMaximumNumberOfPeers();
    if (maxPeers == 0)
        return 9999;

    for (unsigned index = 0; index < maxPeers; index++)
    {
        RakNetGUID g = rakPeerInterface->GetGUIDFromIndex(index);
        if (g != UNASSIGNED_RAKNET_GUID)
        {
            int avePing = rakPeerInterface->GetAveragePing(rakPeerInterface->GetGUIDFromIndex(index));
...
size_t Router2::GetConnectionRequestIndex(RakNetGUID endpointGuid)
{
    for (size_t i = 0; i < connectionRequests.Size(); i++)
    {
...
size_t Router2::ConnnectRequest::GetGuidIndex(RakNetGUID guid)
{
    for (size_t i = 0; i < connectionRequestSystems.Size(); i++)
    {
