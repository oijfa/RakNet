...
void NatTypeDetectionServer::Update(void)
{
    ...
    while (recvStruct)
    {
        ...
        if (data[0] == NAT_TYPE_PORT_RESTRICTED && recvStruct->socket == s3p4)
        {
            ...
            if (readSuccess)
            {
                size_t i = GetDetectionAttemptIndex(senderGuid);
                if (i != (size_t) -1)
                {
                    bs.Reset();
        ...
        if (time > natDetectionAttempts[i].nextStateTime)
        {
            ...
            switch (natDetectionAttempts[i].detectionState)
            {
                ...
                case STATE_TESTING_NONE_2:
                    ...
                    // SocketLayer::SendTo_PC( s4p5, (const char*) &c, 1, saOut  );
                    bsp.data = (char *) &c;
                    bsp.length = 1;
                ...
                case STATE_TESTING_FULL_CONE_2:
                    ...
                    // SocketLayer::SendTo_PC( s2p3, (const char*) bs.GetData(), bs.GetNumberOfBytesUsed(), saOut  );
                ...
                case STATE_TESTING_ADDRESS_RESTRICTED_2:
                    ...
                    saOut = natDetectionAttempts[i].systemAddress;
                    saOut.SetPortHostOrder(natDetectionAttempts[i].systemAddress.GetPort());
                    //SocketLayer::SendTo_PC( s1p2, (const char*) bs.GetData(), bs.GetNumberOfBytesUsed(), saOut  );
                    bsp.data = (char *) bs.GetData();
                    bsp.length = bs.GetNumberOfBytesUsed();
                    bsp.systemAddress = saOut;
                    s1p2->Send(&bsp);
                ...
                case STATE_TESTING_PORT_RESTRICTED_2:
                    ...
                    rakPeerInterface->Send(&bs, HIGH_PRIORITY, RELIABLE, 0, natDetectionAttempts[i].systemAddress,
                                           false);
                    break;
                default:
                                                ...
                                                PI2_LostConnectionReason lostConnectionReason)
{
    ...
    size_t i = GetDetectionAttemptIndex(systemAddress);
    if (i == (size_t) -1)
        return;
    natDetectionAttempts.RemoveAtIndexFast(i);
}
...
{
    size_t i = GetDetectionAttemptIndex(packet->systemAddress);
...
size_t NatTypeDetectionServer::GetDetectionAttemptIndex(const SystemAddress &sa)
{
    for (size_t i = 0; i < natDetectionAttempts.Size(); i++)
    {
...
size_t NatTypeDetectionServer::GetDetectionAttemptIndex(RakNetGUID guid)
{
    for (size_t i = 0; i < natDetectionAttempts.Size(); i++)
    {
