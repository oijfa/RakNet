...
void TeamBalancer::SetForceEvenTeams(bool force)
{
    ...
    if (forceTeamsToBeEven)
        EvenTeams(); // Run the even team algorithm
}
...
void TeamBalancer::SetLockTeams(bool lock)
{
    ...
    if (!lockTeams)
    {
        ...
        for (TeamId i = 0; i < teamMembers.Size(); i++)
        {
            ...
            {
                for (TeamId j = i + 1; j < teamMembers.Size(); j++)
                {
...
void TeamBalancer::RequestSpecificTeam(NetworkID memberId, TeamId desiredTeam)
{
    ...
    for (size_t i = 0; i < myTeamMembers.Size(); i++)
    {
...
void TeamBalancer::RemoveByGuid(RakNetGUID rakNetGUID)
{
    ...
    if (WeAreHost())
    {
        for (size_t droppedMemberIndex = 0; droppedMemberIndex < teamMembers.Size();)
        {
            if (teamMembers[droppedMemberIndex].memberGuid == rakNetGUID)
            {
                ...
                {
                    if (forceTeamsToBeEven)
                        EvenTeams(); // - teams were forced to be even, then run the even team algorithm
                    else if (teamMemberCounts[droppedTeam] == teamLimits[droppedTeam] - 1)
                    {
                        // - teams were NOT forced to be even, and the team the dropped player on was full,
                        // then move users wanting to join that team (if any)
...
void TeamBalancer::OnStatusUpdateToNewHost(Packet *packet)
{
    ...
    for (uint8_t i = 0; i < requestedTeamChangeListSize; i++)
    {
        ...
        size_t memberIndex = GetMemberIndex(tm.memberId, packet->guid);
        if (memberIndex == (size_t) -1)
        {
            ...
            if (tm.currentTeam == UNASSIGNED_TEAM_ID)
            {
                ...
                if (tm.requestedTeam == UNASSIGNED_TEAM_ID)
                    tm.currentTeam = GetNextDefaultTeam(); // Assign a default team
                else
                {
...
void TeamBalancer::OnCancelTeamRequest(Packet *packet)
{
    ...
    size_t memberIndex = GetMemberIndex(memberId, packet->guid);
    if (memberIndex != (size_t) -1)
...
void TeamBalancer::OnRequestSpecificTeam(Packet *packet)
{
    ...
    else
    {
        ...
        else
        {
            ...
            size_t swappableMemberIndex = 0;
            for (; swappableMemberIndex < teamMembers.Size(); swappableMemberIndex++)
            {
            ...
            if (swappableMemberIndex != teamMembers.Size())
            {
                ...
                NotifyTeamSwitchPending(packet->guid, tm.requestedTeam, tm.memberId); // Full or would not be even
        }
    }
}

size_t TeamBalancer::GetMemberIndex(NetworkID memberId, RakNetGUID guid) const
{
    for (size_t i = 0; i < teamMembers.Size(); i++)
    {
...
size_t TeamBalancer::AddTeamMember(const TeamMember &tm)
{
...
void TeamBalancer::RemoveTeamMember(size_t index)
{
...
void TeamBalancer::EvenTeams(void)
{
    ...
    for (TeamId teamMemberCountsIndex = 0; teamMemberCountsIndex < teamMemberCounts.Size(); teamMemberCountsIndex++)
    {
               ...
               teamMemberCounts[teamMemberCountsIndex] < teamLimits[teamMemberCountsIndex])
        {
            ...
            size_t memberIndexToSwitch = GetMemberIndexToSwitchTeams(overpopulatedTeams, teamMemberCountsIndex);
...
size_t TeamBalancer::GetMemberIndexToSwitchTeams(const DataStructures::List<TeamId> &sourceTeamNumbers, TeamId targetTeamNumber)
{
    DataStructures::List<size_t> preferredSwapIndices;
    DataStructures::List<size_t> potentialSwapIndices;
    for (size_t j = 0; j < sourceTeamNumbers.Size(); j++)
    {
...
void TeamBalancer::SwitchMemberTeam(size_t teamMemberIndex, TeamId destinationTeam)
{
...
void TeamBalancer::NotifyTeamAssigment(size_t teamMemberIndex)
{
...
bool TeamBalancer::TeamWouldBeOverpopulatedOnAddition(TeamId teamId, size_t teamMemberSize)
{
    ...
    if (forceTeamsToBeEven)
    {
        size_t allowedLimit = teamMemberSize / teamLimits.Size() + 1;
...
bool TeamBalancer::TeamWouldBeUnderpopulatedOnLeave(TeamId teamId, size_t teamMemberSize)
{
    if (forceTeamsToBeEven)
    {
        size_t minMembersOnASingleTeam = (teamMemberSize - 1) / teamLimits.Size();
...
TeamId TeamBalancer::GetSmallestNonFullTeam(void) const
{
    ...
    for (TeamId idx = 0; idx < teamMemberCounts.Size(); idx++)
    {
...
{
    RakAssert(!forceTeamsToBeEven && !lockTeams);
...
TeamId TeamBalancer::MoveMemberThatWantsToJoinTeamInternal(TeamId teamId)
{
    ...
    {
        size_t swappedMemberIndex = membersThatWantToJoinTheTeam[randomMT() % membersThatWantToJoinTheTeam.Size()];
        TeamId oldTeam = teamMembers[swappedMemberIndex].currentTeam;
...
void TeamBalancer::SwapTeamMembersByRequest(size_t memberIndex1, size_t memberIndex2)
{
...
bool TeamBalancer::TeamsWouldBeEvenOnSwitch(TeamId t1, TeamId t2)
{
    ...
    size_t size = teamMembers.Size() - 1;
    return !TeamWouldBeOverpopulatedOnAddition(t1, size) && !TeamWouldBeUnderpopulatedOnLeave(t2, size);
}
