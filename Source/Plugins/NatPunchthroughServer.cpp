...
{
    size_t index = connectionAttempts.GetIndexOf(ca);
    if (index != (size_t) -1)
    {
        delete ca;
...
{
    for (size_t index = 0; index < connectionAttempts.Size(); index++)
    {
...
NatPunchthroughServer::~NatPunchthroughServer()
{
    ...
    {
        user = users[0];
        for (size_t j = 0; j < user->connectionAttempts.Size(); j++)
        {
...
void NatPunchthroughServer::Update(void)
{
    ...
    if (time > lastUpdate + 250)
    {
        ...
        for (size_t i = 0; i < users.Size(); i++)
        {
            ...
            for (size_t j = 0; j < user->connectionAttempts.Size(); j++)
            {
                ConnectionAttempt *connectionAttempt = user->connectionAttempts[j];
                if (connectionAttempt->sender == user)
                {
                    if (connectionAttempt->attemptPhase != ConnectionAttempt::NAT_ATTEMPT_PHASE_NOT_STARTED &&
                        time > connectionAttempt->startTime && time > 10000 + connectionAttempt->startTime) // Formerly 5000, but sometimes false positives
                    {
...
PluginReceiveResult NatPunchthroughServer::OnReceive(Packet *packet)
{
    switch (packet->data[0])
    {
        ...
        case ID_NAT_REQUEST_BOUND_ADDRESSES:
        {
            ...
            for (size_t i = 0; i < boundAddressCount; i++)
        ...
        case ID_OUT_OF_BAND_INTERNAL:
            if (packet->length >= 2 && packet->data[1] == ID_NAT_PING)
            {
                ...
                rakPeerInterface->SendOutOfBand(packet->systemAddress.ToString(false), packet->systemAddress.GetPort(),
                                               ...
                                               PI2_LostConnectionReason lostConnectionReason)
{
    ...
    bool objectExists;
    size_t i = users.GetIndexFromKey(rakNetGUID, &objectExists);
    if (objectExists)
    {
        RakNet::BitStream outgoingBs;
        ...
        User *otherUser;
        ConnectionAttempt *connectionAttempt;
        for (size_t connectionAttemptIndex = 0; connectionAttemptIndex < user->connectionAttempts.Size(); connectionAttemptIndex++)
        {
...
void NatPunchthroughServer::OnNATPunchthroughRequest(Packet *packet)
{
    ...
    bool objectExists;
    size_t i = users.GetIndexFromKey(senderGuid, &objectExists);
...
{
    bool objectExists;
    size_t i = users.GetIndexFromKey(packet->guid, &objectExists);
    if (objectExists)
    {
...
void NatPunchthroughServer::StartPunchthroughForUser(User *user)
{
    ...
    for (size_t i = 0; i < user->connectionAttempts.Size(); i++)
    {
