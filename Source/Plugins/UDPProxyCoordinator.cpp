...
void UDPProxyCoordinator::Update(void)
{
    ...
    for (size_t idx = 0; idx < forwardingRequestList.Size();)
    {
        ForwardingRequest *fw = forwardingRequestList[idx];
...
void UDPProxyCoordinator::OnForwardingRequestFromClientToCoordinator(Packet *packet)
{
    ...
    size_t insertionIndex = forwardingRequestList.GetIndexFromKey(sata, &objectExists1);
    ...
    if (rakPeerInterface->GetConnectionState(targetAddress) != IS_CONNECTED && !usesAddress)
    {
        outgoingBs.Write((MessageID) ID_UDP_PROXY_GENERAL);
    ...
    if (serverList.Size() > 1)
    {
        ...
        for (size_t idx = 0; idx < serverList.Size(); idx++)
        ...
        for (size_t copyIndex = 0; copyIndex < serverList.Size(); copyIndex++)
...
void UDPProxyCoordinator::OnLoginRequestFromServerToCoordinator(Packet *packet)
{
    ...
    size_t insertionIndex = serverList.GetIndexOf(packet->systemAddress);
    if (insertionIndex != (size_t) -1)
    {
        outgoingBs.Write((MessageID) ID_UDP_PROXY_GENERAL);
...
void UDPProxyCoordinator::OnForwardingReplyFromServerToCoordinator(Packet *packet)
{
    ...
    bool objectExists;
    size_t index = forwardingRequestList.GetIndexFromKey(sata, &objectExists);
    if (!objectExists)
        return; // The guy disconnected before the request finished
    ...
    unsigned char c;
    incomingBs.Read(c);
    UDPForwarderResult success = (UDPForwarderResult) c;
...
void UDPProxyCoordinator::OnPingServersReplyFromClientToCoordinator(Packet *packet)
{
    ...
    {
        for (size_t idx = 0; idx < serversToPingSize; idx++)
        {
            incomingBs.Read(swp.serverAddress);
            incomingBs.Read(swp.ping);

            for (size_t index2 = 0; index2 < fw->sourceServerPings.Size(); index2++)
            {
    ...
    else
    {
        for (size_t idx = 0; idx < serversToPingSize; idx++)
        {
            ...
            for (size_t index2 = 0; index2 < fw->targetServerPings.Size(); index2++)
            {
...
void UDPProxyCoordinator::TryNextServer(SenderAndTargetAddress sata, ForwardingRequest *fw)
{
    ...
    while (fw->remainingServersToTry.Size() > 0)
    {
        ...
        if (serverList.GetIndexOf(fw->currentlyAttemptedServerAddress) != (size_t) -1)
        {
...
void UDPProxyCoordinator::Clear(void)
{
    ...
    for (size_t i = 0; i < forwardingRequestList.Size(); i++)
