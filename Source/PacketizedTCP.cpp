...
{
    TCPInterface::Stop();
    for (unsigned i = 0; i < waitingPackets.Size(); i++)
        DeallocatePacket(waitingPackets[i]);
    ClearAllConnections();
}

void PacketizedTCP::Send(const char *data, size_t length, const SystemAddress &systemAddress, bool broadcast)
{
    PTCPHeader dataLength = length;
...
bool PacketizedTCP::SendList(const char **data, const size_t *lengths, size_t numParameters, const SystemAddress &systemAddress, bool broadcast)
{
    if (isStarted == 0)
        return false;
    if (data == 0)
    ...
    size_t lengthsArray[512];
    const char *dataArray[512];
    dataArray[0] = (char *) &dataLength;
    lengthsArray[0] = sizeof(dataLength);
    for (unsigned i = 0; i < 512 && i < numParameters; i++)
    {
...
{
    SystemAddress sa = TCPInterface::HasNewIncomingConnection();
...
Packet *PacketizedTCP::ReturnOutgoingPacket(void)
{
    ...
    while (outgoingPacket == 0 && !waitingPackets.IsEmpty())
    {
