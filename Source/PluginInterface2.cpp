...
void PluginInterface2::SendUnified(const char *data, const size_t length, PacketPriority priority,
                                   PacketReliability reliability, char orderingChannel,
                                   const AddressOrGUID systemIdentifier, bool broadcast)
{
    if (rakPeerInterface)
...
Packet *PluginInterface2::AllocatePacketUnified(size_t dataSize)
{
    if (rakPeerInterface)
    {
...
bool PluginInterface2::SendListUnified(const char **data, const size_t *lengths, size_t numParameters,
                                       ...
                                       const AddressOrGUID systemIdentifier, bool broadcast)
{
    if (rakPeerInterface)
        ...
        return tcpInterface->SendList(data, (const size_t *) lengths, numParameters, systemIdentifier.systemAddress, broadcast);
#endif
    ...
    if (!broadcast && systemIdentifier.rakNetGuid == GetMyGUIDUnified())
    {
        ...
        for (size_t i = 0, lengthOffset = 0; i < numParameters; i++)
        {
            if (lengths[i] > 0)
            {
