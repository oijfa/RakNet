...
void DynDNS::Update(void)
{
    ...
    if (serverAddress != UNASSIGNED_SYSTEM_ADDRESS)
    {
        ...
        else
        {
            ...
            tcp->Send(getString.C_String(), getString.GetLength(), serverAddress, false);
        }
    ...
    if (packet)
    {
        if (connectPhase == CP_WAITING_FOR_DYNDNS_RESPONSE)
        {
            ...
            char *result = strstr((char *) packet->data, "Connection: close");
            if (result != 0)
            {
        ...
        else
        {
            ...
            */
            ...
            char *result = strstr((char *) packet->data, "Current IP Address: ");
            if (result != 0)
            {
    ...
    if (tcp->HasLostConnection() != UNASSIGNED_SYSTEM_ADDRESS && connectPhase == CP_WAITING_FOR_DYNDNS_RESPONSE)
