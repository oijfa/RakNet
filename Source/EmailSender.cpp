...
const char *EmailSender::Send(const char *hostAddress, unsigned short hostPort, const char *sender, const char *recipient,
                              ...
                              FileList *attachedFiles, bool doPrintf, const char *password)
{
    ...
    SystemAddress emailServer = tcpInterface.Connect(hostAddress, hostPort, true);
...
#pragma warning(disable:4127)   // conditional expression is constant
    ...
    char *newBody = (char *) malloc(bodyLength * 3);
    if (bodyLength >= 0)
        newBody[0] = body[0];

    size_t j = 1;
    for (size_t i = 1; i < bodyLength; i++)
    {
        // Transform \n . \r \n into \n . . \r \n
        if (i < bodyLength - 2 &&
            body[i - 1] == '\n' &&
    ...
    if (attachedFiles && attachedFiles->fileList.Size())
    {
        for (size_t i = 0; i < attachedFiles->fileList.Size(); i++)
        {
            // Write boundary
            sprintf(query, "\r\n--%s\r\n", boundary);
            tcpInterface.Send(query, (unsigned int) strlen(query), emailServer, false);
            ...
            sprintf(query,
                    "Content-Type: APPLICATION/Octet-Stream; SizeOnDisk=%li; name=\"%s\"\r\n"
            ...
            outputOffset = Base64Encoding((const unsigned char *) attachedFiles->fileList[i].data,
                                          attachedFiles->fileList[i].dataLengthBytes, newBody);
...
#pragma warning( disable : 4127 ) // warning C4127: conditional expression is constant
    ...
    while (1)
    {
        ...
        RakNet::Packet *packet = tcpInterface->Receive();
        if (packet)
        {
            if (doPrintf)
            {
